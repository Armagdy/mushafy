export interface Surah {
  ...
  startingAyah: number; // Index of the first ayah in the Quran (0-based, total 6236 ayahs)
  revelationOrder: number; // Chronological order of revelation (1-114, e.g., Al-Alaq=1, An-Nasr=114)
  quarter: number; // Number of quarters (hizb quarters) in the surah
}

// Data sourced from quran-data.json which contains comprehensive Quranic metadata
// Data format: [startingAyah, numberOfAyahs, revelationOrder, quarter, name, englishName, englishNameTranslation, revelationType]
const loadSurahs = (): Surah[] => {
  // Data is embedded inline for build-time compatibility and type safety
  const surahsData: [number, number, number, number, string, string, string, string][] = [
    ...
    [ 6168, 8, 16, 1, "التكاثر", "At-Takaathur", "The Rivalry in world increase", "Meccan" ],
    [ 6176, 3, 13, 1, "العصر", "Al-Asr", "The Declining Day", "Meccan" ],
  ...
  return surahsData.map((data, index) => ({
    id: index + 1,
    startingAyah: data[0],
    numberOfAyahs: data[1],
    revelationOrder: data[2],
    quarter: data[3],
    name: data[4],
    englishName: data[5],
    englishNameTranslation: data[6],
    revelationType: data[7] as 'Meccan' | 'Medinan',
  }));
};

export const surahs: Surah[] = loadSurahs();
