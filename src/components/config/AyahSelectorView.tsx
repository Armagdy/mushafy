...
export default function AyahSelectorView({
  ...
  // Auto-scroll to currently playing ayah when view opens or ayah changes
  useEffect(() => {
    if (currentPlayingAyah && ayahListRef.current) {
      const ayahSelector = `[data-ayah="${currentPlayingAyah.surah}-${currentPlayingAyah.ayah}"]`;
      const ayahButton = ayahListRef.current.querySelector(ayahSelector);
      
      if (ayahButton) {
        ayahButton.scrollIntoView({ behavior: 'auto', block: 'center' });
      }
    }
  }, [currentPlayingAyah]);
  ...
  // Get surahs to show based on current page(s)
  const getSurahsToShow = () => {
  ...
  const surahsToShow = getSurahsToShow();

  return (
    <div className="flex flex-col h-full p-4">
      {/* Back Button with Bold Title */}
      <button
        onClick={onClose}
        className={cn(
          "flex items-center gap-2 text-emerald-700 dark:text-emerald-400 hover:text-emerald-800 dark:hover:text-emerald-300 transition-colors mb-4",
      ...
      >
        ...
        <span className="font-bold">{t('selectAyahToPlay')}</span>
      </button>
      ...
      >
        ...
        >
          {surahsToShow.map((surah: any) => (
            <div 
              key={surah.number} 
              className="border border-emerald-200 dark:border-emerald-800 rounded-lg overflow-hidden bg-transparent"
            >
              {/* Surah Header */}
              <div className="bg-emerald-50 dark:bg-emerald-900/20 px-3 py-2 border-b border-emerald-100 dark:border-emerald-900">
                <span className={cn(
              ...
              <div className="overflow-y-auto p-2 grid grid-cols-5 gap-1">
                {surah.verses?.map((verse: any) => (
                  <motion.button
                    ...
                    className={cn(
                      "w-full aspect-square rounded-lg flex items-center justify-center font-medium transition-all border",
                      textSizeClasses.text,
                      currentPlayingAyah?.surah === surah.number && currentPlayingAyah?.ayah === verse.number
                        ? 'bg-gradient-to-r from-emerald-500 to-teal-500 text-white shadow-md border-emerald-600'
                        : 'bg-white dark:bg-gray-800 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 text-emerald-800 dark:text-emerald-200 border-gray-300 dark:border-gray-700'
                    )}
                  >
